...
void print(const char *name, const char *fname, const char *title, int chanel)
{
	...
	for(Long64_t event = 0; event < eventNr; event++)
	{
		...
		if(cnt4_eeuu > 0) {
			CNT4_eeuu += weight1 * weight2 * weight3;
		}
		if(cnt4_uuee > 0) {
			CNT4_uuee += weight1 * weight2 * weight3;
		}
		if(cnt5_eeuu > 0) {
			CNT5_eeuu += weight1 * weight2 * weight3;
		}
		if(cnt5_uuee > 0) {
			CNT5_uuee += weight1 * weight2 * weight3;
		}
		if(cnt6_eeuu > 0) {
			CNT6_eeuu += weight1 * weight2 * weight3;
		}
		if(cnt6_uuee > 0) {
			CNT6_uuee += weight1 * weight2 * weight3;
		}
		if(cnt7_eeuu > 0) {
			CNT7_eeuu += weight1 * weight2 * weight3;
		}
		if(cnt7_uuee > 0) {
			CNT7_uuee += weight1 * weight2 * weight3;
		}
		if(cnt8_eeuu > 0) {
